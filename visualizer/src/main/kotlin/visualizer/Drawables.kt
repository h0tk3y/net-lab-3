...
public class TextDrawable(val p: Point,
    ...
    override fun draw(graphics: Graphics, coords: Coordinates) {
        graphics.color = color
        val (x0, y0) = coords.getDrawingCoordinates(p)
        val y1 = coords.getDrawingCoordinates(Point(p.x, p.y - size.toDouble() / 100)).second
        graphics.font = Font("TimesRoman", Font.PLAIN, (y1 - y0))
