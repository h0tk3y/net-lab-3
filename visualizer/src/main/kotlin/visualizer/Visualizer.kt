...
public class Visualizer : JPanel() {
    ...
    public var area: Area = Area(Point(0.0, 0.0), Point(1.0, 1.0))
        set(n: Area) {
            ...
            repaint()
            onAreaChangedListener?.let { it(n) }
        }

    public fun changeArea(n: Area) {
        val callback = onAreaChangedListener
        onAreaChangedListener = null
        area = n
        onAreaChangedListener = callback
    }
    ...
    private var onAreaChangedListener: (Visualizer.(Area) -> Unit)? = null
    public fun onAreaChanged(action: Visualizer.(Area) -> Unit) {
        onAreaChangedListener = action
    }
